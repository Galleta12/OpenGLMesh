...
Mesh::Mesh(std::vector<Vertex> &vertices, std::vector<GLuint> &indices, std::vector<Texture> &textures)
{
    ...
    EBO EBO(indices);
	// Links VBO attributes such as coordinates and colors to mVAO->	mVAO->LinkAttrib(VBO, 0, 3, GL_FLOAT, sizeof(Vertex), (void*)0);
...
)
{
	...
	shader.set_eye_position(camera.Position.x, camera.Position.y, camera.Position.z);

	
    
    camera.drawComponent(shader);
